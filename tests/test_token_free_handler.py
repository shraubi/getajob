...
class TokenFreeHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hirify_job_reports_raw_telegram_contact_and_confirmation(self):
        with tempfile.TemporaryDirectory() as directory:
            resume = Path(directory) / "resume.pdf"
            resume.write_bytes(b"pdf")
            ...
            with patch("bot.handlers.fetch_job_from_message", new=AsyncMock(return_value=page)), \
                 ...
                 patch("bot.handlers.HirifyClient", FakeHirifyClient), \
                 patch("bot.handlers.build_application_for_vacancy", return_value=draft), \
                 patch("bot.handlers.save_fetched_job", return_value="a" * 64), \
                 patch("bot.handlers.config.HIRIFY_EMAIL", "user@example.com"), \
                 patch("bot.handlers.config.HIRIFY_PASSWORD", "secret"):
                await _handle_token_free(ctx, vacancy.url)
            summary = next(item for item in ctx.bot.messages if "Source: telegram_contact" in item["text"])
            self.assertIn("Contact: telegram:brandiumsu", summary["text"])
