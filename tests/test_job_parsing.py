...
from token_free import format_vacancy_summary, parse_vacancy, render_message
...
class JobParsingTests(unittest.TestCase):
    ...
    def test_response_uses_skills_and_not_salary_as_company(self):
        vacancy = parse_vacancy(LEAD)
        message = render_message(vacancy, "data_engineering")
        self.assertIn("python, sql, etl, data modeling", message)
        self.assertNotIn("at 47 000", message)
