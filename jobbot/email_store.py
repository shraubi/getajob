    ...
    try:
        ...
        connection.execute(
            """UPDATE inbound_email_messages
               SET discovered_count=?, duplicate_count=?, handled_at=? WHERE id=?""",
            (len(offers), duplicates, now, receipt_id),
        )
        connection.commit()
    ...
    try:
        cursor = connection.execute(
            """INSERT OR IGNORE INTO inbound_email_messages (
                   mailbox_key, uid_validity, uid, message_id_hash, content_hash,
                   status, last_error, first_seen_at, handled_at
               ) VALUES (?, ?, ?, ?, ?, 'rejected', ?, ?, ?)""",
            (
                mailbox_key, uid_validity, uid, _digest(message_id),
                _digest(raw_message), reason[:500], now, now,
            ),
        )
        connection.commit()
