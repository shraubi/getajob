...
def render_message(vacancy: Vacancy, direction: str) -> str:
    role = vacancy.title
    target = f" at {vacancy.company}" if vacancy.company != "Unknown company" else ""
    if vacancy.skills:
        concrete = ", ".join(vacancy.skills[:4])
        fit = f"My experience with {concrete} aligns well with the role."
    else:
        fit = f"My {_DIRECTION_LABELS.get(direction, direction)} background aligns well with the role."
    url_line = f"\n\nVacancy: {vacancy.url}" if vacancy.url else ""
    return (
        f"Hi! I'm applying for the {role} role{target}. {fit} "
        "I've attached the most relevant version of my resume and would be glad to discuss the team's priorities."
        f"{url_line}"
    )
